                    ...
                    <?php else : ?>
                        <ul>
                            <?php foreach ($cartSummary['items'] as $item) :
                                ...
                                $itemTotal = $unitPrice * $item['so_luong'];
                            ?>
                                <li class="minicart-item">
                                    <div class="minicart-thumb">
                                        <a href="<?= BASE_URL . '?act=chi-tiet-san-pham&id_san_pham=' . $item['san_pham_id'] ?>">
                                            <img src="<?= BASE_URL . $item['hinh_anh'] ?>" alt="<?= htmlspecialchars($item['ten_san_pham']) ?>">
                                        </a>
                                    </div>
                                    <div class="minicart-content">
                                        <h3 class="product-name">
                                            <a href="<?= BASE_URL . '?act=chi-tiet-san-pham&id_san_pham=' . $item['san_pham_id'] ?>"><?= htmlspecialchars($item['ten_san_pham']) ?></a>
                                        </h3>
                                        <p>
                                            <span class="cart-quantity"><?= intval($item['so_luong']) ?> <strong>&times;</strong></span>
                                            <span class="cart-price"><?= formatPrice($itemTotal) ?> đ</span>
                                        </p>
                                    </div>
                                    <a class="minicart-remove" href="<?= BASE_URL . '?act=xoa-san-pham-gio-hang&id_san_pham=' . $item['san_pham_id'] ?>" onclick="return confirm('Bạn có chắc muốn xoá sản phẩm này khỏi giỏ hàng?');"><i class="pe-7s-close"></i></a>
                                </li>
                            <?php endforeach; ?>
                        </ul>
                    <?php endif; ?>
